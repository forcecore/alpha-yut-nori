...
class Board:
    ...
    # Backward movement (back-do): BACK_DO[position] = list of possible previous positions.
...
15 16 17  18 19 00"""
    ...
    def get_next_position(self, current_pos: str, steps: int) -> Optional[str]:
        ...
        if steps == 0:
            return current_pos
        if steps == -1:
            dests = self.BACK_DO.get(current_pos, [])
            return dests[0] if dests else None
        return self.MOVE_TABLE.get(current_pos, {}).get(steps)

    def get_back_do_destinations(self, current_pos: str) -> List[str]:
        """Return all possible back-do destinations (may be >1 at merge points)."""
        return self.BACK_DO.get(current_pos, [])
